...
# Display-only score calculation (shown on radar blip labels)
...
func _fmts(v: int) -> String:
  if v>=1000000: return "%s" % _comma(v)
...
func _draw_bold(pos: Vector2, text: String, sz: int, col: Color) -> void:
  for ox in [-1, 0, 1]:
    for oy in [-1, 0, 1]:
      draw_string(df, pos + Vector2(ox, oy), text, HORIZONTAL_ALIGNMENT_LEFT, -1, sz, col)

func _ready() -> void:
  rr = size.y * 0.47
  rc = Vector2(rr + 16, size.y * 0.5)
  px = rc.x + rr + 24
  py_top = 4.0
  pw = size.x - px - 6
  ph = size.y - py_top - 4
...
func _process(delta: float) -> void:
  ...
  for i in range(explosions.size()-1,-1,-1):
    ...
    if explosions[i]["t"] <= 0: explosions.remove_at(i)
  match gs:
...
func _draw_radar() -> void:
  ...
  for id in ms:
    ...
    if br>0.1:
      draw_string(df,p+Vector2(-12,bR+14),MIG_DEFS[id]["label"],HORIZONTAL_ALIGNMENT_LEFT,-1,11,Color(0.3,0.8,0.3,max(br,0.35)))
      draw_string(df,p+Vector2(-16,bR+25),_fmts(_mscore(id)),HORIZONTAL_ALIGNMENT_LEFT,-1,10,Color(0.5,1,0.3,max(br,0.3)))
  for ex in explosions:
    var prog = 1.0 - (ex["t"] / ex["max_t"])
    var alpha = 1.0 - prog
    draw_arc(ex["pos"], 8.0 + prog * 35.0, 0, TAU, 24, Color(1, 0.5, 0, alpha * 0.8), 2.5 - prog * 2.0)
    if prog < 0.3:
      draw_circle(ex["pos"], 12 * (1.0 - prog), Color(1, 0.9, 0.3, (1.0 - prog / 0.3) * 0.9))
    if prog < 0.6:
      var spark_a = (1.0 - prog / 0.6) * 0.7
      for si in range(6):
        var sa2 = ex["seed"] + si * TAU / 6.0 + prog * 1.5
        var sp = ex["pos"] + Vector2(cos(sa2), sin(sa2)) * (5.0 + prog * 30.0)
        draw_circle(sp, 2.0 - prog * 2.0, Color(1, 0.6, 0.1, spark_a))
  draw_circle(rc,3,Color(0,0.8,0,0.8))
  for i in range(20): draw_arc(rc,rr+float(i)*3,0,TAU,64,Color(0,0,0,float(i)/20*0.75),4)

func _draw_panel() -> void:
  var box = Rect2(px - 6, py_top, pw + 8, ph)
  draw_rect(box, Color(0, 0.03, 0, 0.85))
  draw_rect(box, Color(0.35, 0.7, 0.15, 0.5), false, 1.5)
  var cx = px + 6
  var cy = py_top + 20
  var cw = pw - 12
  var ml = max(0, _wsn()-kwav)
...
func _dlockon() -> void:
  ...
  for r in [Rect2(0,0,size.x,5),Rect2(0,size.y-5,size.x,5),Rect2(0,0,5,size.y),Rect2(size.x-5,0,5,size.y)]: draw_rect(r,bc)
  draw_string(df,Vector2(size.x*0.15, size.y*0.25),"MIG LOCKING ON",HORIZONTAL_ALIGNMENT_LEFT,-1,30,Color(1,0.2,0,0.65+fl*0.35))
  draw_string(df,Vector2(size.x*0.45, size.y*0.50),"%d" % int(ceil(lkT)),HORIZONTAL_ALIGNMENT_LEFT,-1,48,Color(1,0.8,0,0.9) if lkT>5 else Color(1,0.1,0,0.6+fl*0.4))
  var bx=size.x*0.1; var by=size.y*0.57; var bw=size.x*0.8; var pc=lkT/LT
  draw_rect(Rect2(bx,by,bw,10),Color(0.12,0.12,0.12,0.7))
  draw_rect(Rect2(bx,by,bw*pc,10),Color(0,0.8,0,0.85) if pc>0.3 else Color(1,0,0,0.5+fl*0.5))
  if evId!="" and evId in MIG_DEFS:
    var ef = abs(sin(lkT*5))
    draw_string(df,Vector2(size.x*0.08, size.y*0.78),"HIT  %s  TO EVADE!" % MIG_DEFS[evId]["full_name"],HORIZONTAL_ALIGNMENT_LEFT,-1,20,Color(0,1,0,0.4+ef*0.6))

func _dsplash() -> void:
  var fl = abs(sin(spT*4))
  draw_rect(Rect2(Vector2.ZERO,size),Color(0.3,0,0,0.35+fl*0.2))
  for r in [Rect2(0,0,size.x,6),Rect2(0,size.y-6,size.x,6),Rect2(0,0,6,size.y),Rect2(size.x-6,0,6,size.y)]: draw_rect(r,Color(1,0,0,0.5+fl*0.4))
  draw_string(df,Vector2(size.x*0.12, size.y*0.35),"YOU'VE BEEN HIT!",HORIZONTAL_ALIGNMENT_LEFT,-1,36,Color(1,0.1,0,0.7+fl*0.3))
  draw_string(df,Vector2(size.x*0.14, size.y*0.55),"PENALTY: %s" % lastPen,HORIZONTAL_ALIGNMENT_LEFT,-1,18,Color(1,0.55,0.15,0.6+fl*0.4))
...
func _clear_all_migs() -> void:
  for mid in ms:
    if ms[mid]["on"]: _dm(mid)

func _spawn_explosion(pos: Vector2) -> void:
  explosions.append({"pos": pos, "t": EXPLOSION_DUR, "max_t": EXPLOSION_DUR, "seed": randf() * TAU})

func _destroy(id: String) -> void:
  if id not in ms or not ms[id]["on"]: return
  var sc = _mscore(id)
  _spawn_explosion(_mp(id))
  _dm(id)
  ktot+=1; kwav+=1; supBank+=sc
  # MPF handles actual scoring via per-MIG variable_player entries
  # Just post the destroyed event, MPF already set multiball_last_jackpot on hit
  _ev("radar_mig_destroyed")
  if kwav>=_wsn() and not supLit:
    supLit=true; _ev("radar_super_jackpot_lit"); _clear_all_migs()
...
func _on_super(_msg = null) -> void:
  ...
  wav+=1; kwav=0; supBank=0; _ev("radar_wave_advance"); _spawn_init()
...
func _on_ending(_msg = null) -> void:
  ...
  gs=GS.PLAY; pens.clear(); explosions.clear()
...
# Read an MPF player variable, with fallback if not available
func _get_mpf_var(varname: String, fallback: int = 0) -> int:
